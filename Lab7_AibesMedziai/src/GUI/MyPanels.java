 ...
 */
//Šios klasės objektu galima sukurti du panelius: parametrų lentelę ir mygtukų 
//tinklelį sudaryti. Panelyje talpinamų objektų kiekis nustatomas parametrais.
public class MyPanels extends JPanel {
    ...
    public MyPanels(String[] lblTexts, String[] tfTexts, int columnWidth) {
        ...
        //Tokia pati parametrų lentelė, padaryta naudojant SpringLayout 
        //išdėstymo dėsnį
    ...
    public JButton[] getButtons() {
        if (btns == null) {
            throw new NullPointerException("No buttons array");
        } else {
            return btns;
        }
    }
    ...
    public String[] getParametersOfTable() {
        if (tfs == null) {
            throw new NullPointerException("No table of parameters");
        } else {
            for (int i = 0; i < tfs.length; i++) {
